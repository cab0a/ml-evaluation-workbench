...
モデル評価の設計や結果の監査方法を確認する機械学習エンジニア向けです。

主な評価・成果物は次のとおりです。

- 分割ごとの評価指標と行単位の予測結果
- 特徴量除去比較、データ分割の整合性検証、ラベルシャッフルによる負の対照実験
- 確率校正、検証方式を変えた場合の安定性、クラス不均衡への感度
- 6種類の実験から固定方針で選んだ25件の代表比較

v1.0.0では数値実験を変更せず、コマンドライン操作、Python API、27個の成果物名、15種類のCSV列順、5種類のJSON最上位キーを1.x安定インターフェースとして固定しました。

前処理の適合には各分割の学習用データだけを使い、成果物はSHA-256とPython 3.12の参照環境で再現性を検証できます。

安定性の範囲、評価結果の適用限界、再現手順の詳細は、以下の英語本文を参照してください。
